...
export function StayFilter({ filterBy, setFilterBy, onSearch }) {
  ...
  }, [filterBy, startDate, endDate, guests])
  ...
  function handleRegionSelect(region) {
    const isContinent = regions.some((reg) => reg.name === region.name && reg.isContinent)
    const fieldName = isContinent ? 'continent' : 'country'
    setFilterToEdit({ ...filterToEdit, [fieldName]: region.name, txt: region.name }) // Updated
    setIsRegionDropdownOpen(false)
  }
              ...
              <div key={type}>
                ...
                <div className="guest-counter">
                  <button
                    onClick={() => handleGuestsChange(type, guests[type] - 1)}
                    disabled={
                      guests[type] === 0 || (type === 'adults' && guests[type] === 0)
                    }
                  >
                    -
                  </button>
